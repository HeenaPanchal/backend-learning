...
if(rowCount === 0){
    throw new Error("Email does not exist")
        ...
        await pool.query(`INSERT INTO otp (
            email,
            otp,
            expiry_date
...
otpRoute.post("/login", async(req,res)=>{
    ...
    try{
        const {rowCount:count ,rows:userRows} = await pool.query(`SELECT uuid , email FROM users WHERE email=$1`,[email])
        if(count===0){
            throw new Error("Email is not registered")
        }
        const {rowCount,rows}  = await pool.query(`SELECT email , otp FROM otp WHERE email=$1`,[email])
        if(rowCount===0){
